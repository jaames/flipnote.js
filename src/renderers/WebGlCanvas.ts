...
export class WebglCanvas implements CanvasInterface {
  ...
  #layerProgram: ProgramInfo; // for drawing renderbuffer w/ filtering
  #upscaleProgram: ProgramInfo; // for drawing renderbuffer w/ filtering
  ...
  #textureTypes = new Map<WebGLTexture, number>();
  #textureSizes = new Map<WebGLTexture, { width: number, height: number }>();
  #frameBufferTextures = new Map<WebGLFramebuffer, WebGLTexture>();

  #applyFirefoxFix = false;
  ...
  #drawLayers(
    ...
    for (let i = 0; i < numLayers; i++) {
      ...
      note.getLayerPixelsRgba(frameIndex, layerIndex, this.#layerTexturePixelBuffer as any, this.#paletteBuffer);
  ...
  #createScreenQuad(x0: number, y0: number, width: number, height: number, xSubdivs: number, ySubdivs: number) {
    if (this.#checkContextLoss()) return;
  ...
  #setBuffersAndAttribs(program: ProgramInfo, buffer: BufferInfo) {
    if (this.#checkContextLoss()) return;
    setBuffersAndAttributes(this.gl, program.attribSetters, buffer);
  }

  #createTexture(type: number, minMag: number, wrap: number, width = 1, height = 1) {
    if (this.#checkContextLoss()) return;
    const gl = this.gl;
  ...
  #useFramebuffer(fb: WebGLFramebuffer, viewX?: number, viewY?: number, viewWidth?: number, viewHeight?: number) {
    if (this.#checkContextLoss()) return;
    const gl = this.gl;
    if (fb === null) {
  ...
  #resizeFramebuffer(fb: WebGLFramebuffer, width: number, height: number) {
    if (this.#checkContextLoss()) return;
